...
4. *What is the `weights = n` parameter doing in `glm()`?*
...
#### 4. What is the `weights = count` parameter doing in `glm()`?
...
When we want to run logistic regression on a dataset generated with experimental design (like the bottle return problem or the squirrels problem), we transform the data in 3 steps:  

1. Initial dataset reporting the number of successes and the number of failures for a given trial in the experiment.
```{r, echo = F}
squirrels
bottle
```

2. Create a binary response variable (y) to regress on and obtain the count of 0's and 1's from the initial dataset.
```{r}
squirrels_transformed
bot2
```

3. Expand the dataframe to reflect the counts of observations.  
